...
trait FunSets extends FunSetsInterface:
   ...
   */
  def singletonSet(elem: Int): FunSet = (other: Int) => other == elem
  ...
  def forall(s: FunSet, p: Int => Boolean): Boolean =
    def iter(a: Int): Boolean =
      if a < bound && (s(a) && p(a) || !s(a))  then
        iter(a + 1)
      else if a < bound && (s(a) && !p(a)) then false
      else true
    iter(-bound)
   ...
   */
  def map(s: FunSet, f: Int => Int): FunSet = newElem => exists(s, oldElem => f(oldElem) == newElem)
