  ...
  def descendingByRetweet: TweetList =
    this match
      case t: NonEmpty =>
        ...
        Cons(tweet, this.remove(tweet).descendingByRetweet)
      case t: Empty => Nil
...
class NonEmpty(val elem: Tweet, val left: TweetSet, val right: TweetSet) extends TweetSet:
  ...
  def unapply: (Tweet, TweetSet, TweetSet) = (elem, left, right)
  ...
  final override def mostRetweeted: Tweet =
    (left, right) match
      case (l: Empty, r: Empty) => elem
      case (l: NonEmpty, r: Empty) => List(l.mostRetweeted, elem).maxBy(_.retweets)
      case (l: Empty, r: NonEmpty) => List(r.mostRetweeted, elem).maxBy(_.retweets)
      case _ => List(left.mostRetweeted, right.mostRetweeted, elem).maxBy(_.retweets)
...
object NonEmpty {
  def unapply(s: NonEmpty): (Tweet, TweetSet, TweetSet) = (s.elem, s.left, s.right)
}
